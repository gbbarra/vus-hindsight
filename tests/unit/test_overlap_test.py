...
def monta_export(con, linhas):
    ...
    """)
    for linha in linhas:
        con.execute("INSERT INTO export VALUES (?, ?, ?, ?, ?)",
                    [linha[c] for c in COLUNAS_EXPORT])


def variante(vid, arm="vus_to_plp", horizonte="18", estrato="primary",
             consequencia="missense"):
    return {"variant_id_hg38": vid, "arm": arm, "horizon_months": horizonte,
            "stratum": estrato, "molecular_consequence": consequencia}


def coorte(n_reclassificadas, n_controle, horizonte="18"):
    """Variantes reclassificadas e o controle que continuou VUS."""
    linhas = [variante(f"chr1_{i}_A_G_hg38", horizonte=horizonte)
              for i in range(n_reclassificadas)]
    linhas += [variante(f"chr2_{i}_A_G_hg38", arm="still_vus",
                        horizonte="still_vus")
...
def test_exatamente_dez_vezes_o_controle_nao_basta(con, tmp_path):
    ...
    ids = ["chr1_0_A_G_hg38", "chr1_1_A_G_hg38",
           "chr2_0_A_G_hg38", "chr2_1_A_G_hg38"]

    resultado = roda(con, tmp_path, linhas, ids)

    assert (resultado["by_arm"]["vus_to_plp"]["pct"],
            resultado["by_arm"]["still_vus"]["pct"]) == (2.0, 0.2)
    assert resultado["verdict"] == "MINIMAL"


def test_acima_de_dez_vezes_o_controle_e_exposicao(con, tmp_path):
    linhas = coorte(100, 1000)
    ids = ["chr1_0_A_G_hg38", "chr1_1_A_G_hg38", "chr1_2_A_G_hg38",
           "chr2_0_A_G_hg38", "chr2_1_A_G_hg38"]
...
def test_lista_na_mesma_montagem_sem_casar_e_ausencia_de_sobreposicao(con,
                                                                     tmp_path):
    linhas = coorte(100, 100)
...
def test_horizontes_saem_em_ordem_numerica_e_nao_alfabetica(con, tmp_path):
    linhas = []
    for h in ("61", "18", "36"):
        linhas += [variante(f"chr1_{h}_{i}_A_G_hg38", horizonte=h)
                   for i in range(10)]
    linhas += coorte(0, 10)
...
def test_o_penhasco_entre_horizontes_e_visivel_na_quebra(con, tmp_path):
    ...
    linhas = ([variante(f"chr1_18_{i}_A_G_hg38", horizonte="18") for i in range(10)]
...
def test_apenas_missense_entra_na_comparacao(con, tmp_path):
    ...
    linhas = ([variante(f"chr1_{i}_A_G_hg38") for i in range(10)]
              + [variante(f"chr3_{i}_A_G_hg38", consequencia="frameshift")
                 for i in range(90)]
              + coorte(0, 100))
...
def test_os_rotulos_carregados_pelas_variantes_casadas_sao_contados(con, tmp_path):
    linhas = coorte(100, 100)
    caminho = escreve_lista(tmp_path, [f"chr1_{i}_A_G_hg38" for i in range(5)],
                            rotulo="pathogenic")
