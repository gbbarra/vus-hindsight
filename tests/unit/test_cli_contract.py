...
COM_ARGPARSE = [
    "03b_extract_mc", "04_transitions", "05b_submission_dates", "07_survival",
    "09_reconstruct", "10_validate_reconstruction", "11_contamination_audit",
    "12_export_for_join", "13_alphamissense_overlap", "14_overlap_test",
    "15_evaluate", "16_dbnsfp_to_scores",
]

# Scripts cujos argumentos obrigatórios não têm valor padrão possível.
COM_ARGUMENTO_OBRIGATORIO = [
    "04_transitions", "07_survival", "09_reconstruct",
    "10_validate_reconstruction", "12_export_for_join",
    "13_alphamissense_overlap", "14_overlap_test", "15_evaluate",
]
...
def test_conversor_do_dbnsfp_com_arquivo_inexistente_recusa(tmp_path):
    resultado = roda("16_dbnsfp_to_scores", "--dbnsfp", str(tmp_path / "nao_existe.gz"),
                     "--export", str(tmp_path / "tambem_nao.csv"))
...
def test_teste_de_sobreposicao_com_lista_inexistente_recusa(tmp_path):
    ...
    resultado = roda("14_overlap_test", "--export", str(export),
                     "--list", f"X:{tmp_path / 'nao_existe.csv'}:id:label")
...
def monta_avaliacao(tmp_path):
    ...
    for i in range(40):
        arm, horizonte = ("vus_to_plp", "18") if i < 20 else ("still_vus",
                                                              "still_vus")
    ...
    scores.write_text("variant_id_hg38,score\n"
...
def test_direcao_de_score_invalida_e_erro_e_nao_um_palpite(tmp_path):
    ...
    resultado = roda("15_evaluate", "--export", str(export),
                     "--scores", f"X:{scores}:variant_id_hg38:score:alto",
                     cwd=str(tmp_path))
...
def test_a_direcao_declarada_decide_o_sentido_da_metrica(tmp_path, direcao, auroc):
    ...
    resultado = roda("15_evaluate", "--export", str(export),
                     "--scores", f"X:{scores}:variant_id_hg38:score:{direcao}",
                     cwd=str(tmp_path))
