...
def roda_auditoria(preditor, auditoria, tmp_path):
    ...
    registro.write_text(yaml.safe_dump({"predictors": [preditor]}), encoding="utf-8")
...
@given(
    parsers.parse(
        "um preditor exposto às variantes reclassificadas nos primeiros {meses:d} meses"
    )
)
def preditor_exposto_no_horizonte(avaliacao, meses, tmp_path):
    ...
    with open(tmp_path / "results" / "_overlap_tests.json", "w") as fh:
        json.dump(
            [
                {
                    "name": "X evaluation set",
                    "verdict": "EXPOSED",
                    "by_arm": {"still_vus": {"pct": 0.0}},
                    "by_horizon": [
                        {"horizon": str(meses), "pct": 89.5},
                        {"horizon": "61", "pct": 0.1},
                    ],
                }
            ],
...
def calcula_desempenho(avaliacao, tmp_path):
    ...
    resultado = _roda(
        ...
        str(export),
        "--scores",
        f"X:{scores}:variant_id_hg38:score:high",
        cwd=str(tmp_path),
    )
    assert resultado.returncode == 0, resultado.stderr
...
def periodo_excluido_e_informado(avaliacao):
    assert (
        f"HEADLINE (excluding ['{avaliacao['horizonte_exposto']}'])"
        in avaliacao["saida"]
    )
