...
def line_chart(path, points, series, y_max, title, y_label, y_fmt="{:.0f}%"):
    ...
    for i in range(6):
        ...
        s.append(
            f'<text x="{ml - 10}" y="{y + 4:.1f}" font-size="11" '
    ...
    for x in xs:
        s.append(
            f'<line x1="{px(x):.1f}" y1="{mt + ph}" x2="{px(x):.1f}" '
            f'y2="{mt + ph + 5}" stroke="{INK}" stroke-width="1"/>'
        )
        s.append(
            f'<text x="{px(x):.1f}" y="{mt + ph + 20}" font-size="11" '
            f'text-anchor="middle" fill="{INK}">{x}</text>'
        )
    s.append(
        f'<text x="{ml + pw / 2:.1f}" y="{H - 12}" font-size="12" '
        f'text-anchor="middle" fill="{INK}">months since the baseline '
        f"snapshot</text>"
    )
    s.append(
        f'<text x="16" y="{mt + ph / 2:.1f}" font-size="12" fill="{INK}" '
    ...
    for idx, (key, label, color) in enumerate(series):
        ...
        s.append(
            f'<text x="{ml + pw + 46}" y="{ly + 4}" font-size="11.5" '
...
def main():
    ...
    line_chart(
        os.path.join(RESULTS, "survival_curve.svg"),
        chart_points,
        [("still", "still VUS", SERIES["still"])],
        100.0,
    ...
    line_chart(
        os.path.join(RESULTS, "reclassified_curve.svg"),
        chart_points,
        [
            ("definitive", "resolved to P/LP or B/LB", SERIES["definitive"]),
            ("plp", "resolved to P/LP", SERIES["plp"]),
        ],
        y_max,
    ...
    L.append(
        f"One cohort, **{n:,} variants**, classified *Uncertain "
    ...
    L.append(
        "**Why hold the cohort fixed.** The transition analysis in "
    ...
    L.append("")
    L.append(
        "Month 0 is definitional rather than measured: the cohort is 100% "
    ...
    L.append(
        "How much usable benchmark material the cohort has yielded at each "
    ...
    if any(b < a for a, b in zip(plps, plps[1:], strict=False)):
        L.append(
            "> **Note:** the P/LP count is not monotonic across these "
        ...
        L.append(
            "Each figure is the cohort's state *at that date* rather than a "
    ...
    L.append("## Caveats\n")
    L.append(
        "- The cohort is fixed, so this removes the composition confound "
        "between baselines — it does **not** remove ascertainment bias. "
        "Variants in well-studied genes still resolve faster.\n"
    )
    L.append(
        "- Molecular consequence, used for the hard stratum, comes from the "
    ...
    L.append(
        "- Review status is read from each endpoint's own snapshot, so the "
