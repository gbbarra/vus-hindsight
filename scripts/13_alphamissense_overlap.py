...
def main():
    ...
    """).fetchall()
    stats = {
        a: {"n": n, "hit": h, "pct": round(100.0 * h / n, 4) if n else None}
        for a, n, h in arms
    }
    ...
    L.append(
        "AlphaMissense states that a ClinVar subset was used for model "
    ...
    L.append(
        "That is measurable rather than merely arguable, because the paper "
    ...
    L.append("## Overlap\n")
    L.append(
        "Missense only on both sides: S5 is a missense benchmark, so "
    ...
    if odds is not None:
        L.append(
            f"Fisher exact, reclassified versus control: "
            f"**OR = {odds:,.0f}**, p = {pval:.3g}.\n"
        )
    L.append(
        "The control is what makes this a signal rather than an artefact. "
    ...
    L.append(
        "Reclassifications are stratified by when the label **first "
    ...
    L.append(
        "**It is exposure through evaluation and calibration, not through "
    ...
    L.append(
        "What it shows is narrower and still consequential: variants whose "
    ...
    L.append(
        "The practical consequence is bounded and specific: for the "
    ...
    L.append("")
    L.append(
        "Supplementary Data S5 accompanies Cheng et al., *Science* 2023, "
        ...
        json.dump(
            {
                ...
                "by_horizon": [
                    {"horizon": h, "n": n, "hit": hit, "pct": round(100.0 * hit / n, 4)}
                    for h, n, hit in horizons
                ],
                "by_stratum": [
                    {"arm": a, "stratum": s, "n": n, "hit": hit}
                    for a, s, n, hit in strata
                ],
            },
            fh,
