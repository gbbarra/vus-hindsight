...
def metrics(y, s):
    ...
    if n_pos < MIN_PER_CLASS or n_neg < MIN_PER_CLASS:
        return {"n_pos": n_pos, "n_neg": n_neg, "auroc": None, "auprc": None,
...
def load_audit():
    ...
    for r in json.load(open(path)):
        ...
        bad = [h["horizon"] for h in r["by_horizon"]
...
def main():
    ...
    ap.add_argument("--stratum", default="primary",
    ...
    for spec in args.scores:
        ...
        if direction not in ("high", "low"):
            print(f"FATAL: direction for {name} must be high or low, got "
        ...
        per_h = {}
        for h in sorted({x for x in hz if x != "still_vus"},
                        key=lambda v: int(v)):
        ...
        for h, m in per_h.items():
            ...
            if m["auroc"] is not None:
                print(f"  +{h:>3} months : AUROC {m['auroc']:.3f}  "
                      f"AUPRC {m['auprc']:.3f}{flag}")
            else:
                print(f"  +{h:>3} months : {m['note']}{flag}")
        if bad_h:
            if clean["auroc"] is not None:
                print(f"  HEADLINE (excluding {sorted(bad_h)}): "
                      f"AUROC {clean['auroc']:.3f}  AUPRC {clean['auprc']:.3f}")
            else:
                print(f"  HEADLINE: {clean['note']}")

        out.append({"predictor": name, "file": os.path.basename(path),
                    "md5": md5(path), "direction": direction,
                    "covered": covered, "coverage_pct": round(100.0 * covered / total, 2),
                    "overall": overall, "by_horizon": per_h,
                    "contaminated_horizons": sorted(bad_h),
                    "headline": clean if bad_h else overall})

    # --- report --------------------------------------------------------------
    L = [f"# Evaluation — stratum `{args.stratum}`\n"]
    L.append(f"Positives: **{n_pos:,}** variants that were VUS at baseline and "
             f"are P/LP now. Negatives: **{n_neg:,}** missense controls that "
             "stayed VUS.\n")
    L.append("Discrimination is reported **per horizon**, because that is the "
             ...
             "that.\n")
    if flagged:
        L.append("Horizons flagged by the overlap tests are excluded from the "
    ...
    for r in out:
        ...
        L.append(f"| {r['predictor']} | {r['coverage_pct']:.1f}% | **{auroc}** "
                 f"| {auprc} | {exc} |")
    L.append("")

    for r in out:
        L.append(f"## {r['predictor']}\n")
        L.append(f"`{r['file']}`, md5 `{r['md5']}`; scores read as "
        ...
        for h, m in r["by_horizon"].items():
            ...
            flag = "**contaminated**" if m["contaminated"] else ""
            L.append(f"| +{h} months | {m['n_pos']:,} | {m['n_neg']:,} "
                     f"| {a} | {p} | {flag} |")
        L.append("")

    L.append("## Caveats\n")
    L.append("- Coverage varies between predictors, so AUROCs are not computed "
             "on identical variant sets. Compare with that in mind, or restrict "
             "to the intersection.\n")
    L.append("- Score direction is declared per predictor rather than inferred. "
             "Inferring it from the data would flip an AUC silently whenever a "
             "predictor genuinely performs below chance.\n")
    L.append("- The controls stayed VUS as of the endpoint, which is not the "
    ...
    with open(os.path.join(RESULTS, f"_evaluation_{args.stratum}.json"), "w") as fh:
        json.dump({"stratum": args.stratum, "n_pos": n_pos, "n_neg": n_neg,
                   "flagged": flagged, "predictors": out}, fh, indent=2)
