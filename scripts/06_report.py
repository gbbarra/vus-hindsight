...
def main():
    ...
    if not paths:
        raise SystemExit(
            "no results/_counts_*.json found — run 04_transitions.py first"
        )
    ...
    L.append(
        f"Generated {date.today().isoformat()} from ClinVar `variant_summary` "
    ...
    if os.path.exists(vcf_stats_path):
        ...
        L.append(
            f"Molecular consequence is read from the `MC` (Sequence Ontology) "
        ...
        L.append(
            "Molecular consequence source: ClinVar VCF `MC` field "
    ...
    for m in metas:
        ...
        L.append(
            f"Baseline VUS cohort (criteria provided): **{m['baseline_vus']:,}** "
        ...
        L.append(
            f"**{m['vus_to_plp']:,}** variants moved from Uncertain significance "
        ...
        if m.get("vus_to_plp_not_in_vcf"):
            L.append(
                f"`not_in_vcf` = {m['vus_to_plp_not_in_vcf']:,} variants have no "
        ...
        if conc.get("matched"):
            L.append(
                f"*Diagnostic:* an independent derivation of consequence from "
        ...
        L.append("")
        L.append(
            f"**Hard stratum** — missense AND review status at least "
    ...
    if os.path.exists(manifest):
        ...
        if len(rows) > 1:
            ...
            L.append(
                "`variant_summary.txt.gz` and `clinvar.vcf.gz` are rolling "
                ...
                table(
                    ...
                    [
                        [
                            r[0],
                            f"`{r[1]}`",
                            f"{int(r[3]):,}" if r[3].isdigit() else r[3],
                            r[4],
                            f"`{r[5]}`",
                        ]
                        for r in rows[1:]
    ...
    L.append("```bash\nscripts/run_all.sh\n```\n")
    L.append(
        "Per-variant records: `results/reclassified_pathogenic.tsv` "
