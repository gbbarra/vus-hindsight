...
def analyse(con, name, path, id_col, label_col):
    ...
    if plp.get("n") and ctl.get("n"):
        odds, pval = fisher_exact(
            [[plp["hit"], plp["n"] - plp["hit"]], [ctl["hit"], ctl["n"] - ctl["hit"]]],
            alternative="greater",
        )
...
def main():
    ...
    for spec in args.list:
        ...
        for h in r["by_horizon"]:
            print(
                f"  +{h['horizon']:>3} months: {h['hit']:>5,} / {h['n']:>5,} "
                f"= {h['pct']:6.2f}%"
            )
    ...
    L.append(
        "Any predictor that publishes the variants it was evaluated or "
    ...
    for r in out:
        L.append(f"## {r['name']}\n")
        L.append(
            f"`{r['file']}`, md5 `{r['md5']}` — {r['list_rows']:,} rows, "
            f"{r['ids_hg38']:,} keyed on GRCh38 and {r['ids_hg19']:,} on "
            f"GRCh37.\n"
        )
        if r["verdict"].startswith("UNUSABLE"):
            L.append(
                "**This list cannot be tested.** Its identifiers are "
        ...
        for h in r["by_horizon"]:
            L.append(
                f"| +{h['horizon']} months | {h['hit']:,} | {h['n']:,} "
                f"| **{h['pct']:.2f}%** |"
            )
        L.append("")
        if r["labels"]:
            L.append(
                "Labels carried by the matches: "
    ...
    L.append(
        "A high rate in the reclassified arm alongside a near-zero rate in "
        "the control is exposure. Comparable rates in both would instead "
        "mean the list simply covers a lot of variants.\n"
    )
    L.append(
        "A cliff across horizons dates the snapshot: the label was already "
