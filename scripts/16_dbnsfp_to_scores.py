...
def agg_expr(col, how):
    ...
    parts = (
        f"list_filter(list_transform(str_split(COALESCE({q(col)}, ''), ';'),"
        f" x -> TRY_CAST(x AS DOUBLE)), x -> x IS NOT NULL)"
    )
    return {
        "min": f"list_min({parts})",
        "max": f"list_max({parts})",
        "mean": f"list_avg({parts})",
    }[how]
...
def main():
    ...
    ap.add_argument(
        "--hg38-pos-col", help="override detection: column holding the GRCh38 position"
    )
    ...
    if not args.dbnsfp:
        print(
            "FATAL: --dbnsfp is required (or --peek to inspect a file)", file=sys.stderr
        )
        return 1
    ...
    for p in PREDICTORS:
        ...
        if not col:
            ...
            continue
        how = (
            {"damaging": p["agg"], "mean": "mean"}[args.aggregate]
            if args.raw
            else "max"
        )
        chosen.append(
    ...
    if n_join == 0:
        sample_db = [
            r[0]
            for r in con.execute("SELECT variant_id_hg38 FROM hits LIMIT 3").fetchall()
        ]
        sample_co = [
            r[0]
            for r in con.execute(
                "SELECT variant_id_hg38 FROM cohort LIMIT 3"
            ).fetchall()
        ]
        print(
    ...
    specs = [
        f'--scores "{r["predictor"]}:{r["file"]}:variant_id_hg38:score:'
        f'{r["direction"]}"'
        for r in report
    ]
    ...
    if args.raw:
        ...
        L.append(
            "dbNSFP rankscores rather than raw scores. They are monotone "
    ...
    for r in report:
        L.append(
            f"| {r['predictor']} | `{r['column']}` | {r['scored']:,} "
    ...
    if missing:
        L.append(
            "Not present in this dbNSFP build: "
            + ", ".join(f"**{m}**" for m in missing)
            + ". Absent is recorded as absent; nothing is substituted.\n"
        )
    if empty:
        L.append(
            "Present but empty across the cohort: "
            + ", ".join(f"**{e['predictor']}** (`{e['column']}`)" for e in empty)
